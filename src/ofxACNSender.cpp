...
void ofxACNSender::RecalculateGamma(void)
{
#ifdef OLDGAMMA
	for (int i = 0; i < 256; i++) {
		gamma[i][0] = int(pow(float(i) / 255.0, m_fGamma) * m_iRedMod);
		gamma[i][1] = int(pow(float(i) / 255.0, m_fGamma) * m_iGreenMod);
		gamma[i][2] = int(pow(float(i) / 255.0, m_fGamma) * m_iBlueMod);
	}
#else
	auto fGamma = lgamma(m_fGamma);
	for (int i = 0; i < 256; i++) {
		float fVal = (float)i / 255.f;

		gamma[i][0] = pow(fVal, m_fGamma) * m_iRedMod;
		gamma[i][1] = pow(fVal, m_fGamma) * m_iGreenMod;
		gamma[i][2] = pow(fVal, m_fGamma) * m_iBlueMod;
	}
#endif // OLDGAMMA
...
void ofxACNSender::SetGammaValues(float _fGamma, int redMod, int greenMod, int blueMod)
{
	m_fGamma = _fGamma;
	m_iRedMod = redMod;
	m_iGreenMod = greenMod;
	m_iBlueMod = blueMod;
...
ofColor ofxACNSender::setGamma(ofColor in) const
{
	in.r = gamma[in.r][0];
	in.g = gamma[in.g][1];
	in.b = gamma[in.b][2];
